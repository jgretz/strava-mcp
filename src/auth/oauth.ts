...
export async function authorize(): Promise<Result<{ token: AuthToken; athleteName: string }, string>> {
  ...
  return new Promise((resolve) => {
    ...
    const authUrl = `${STRAVA_AUTH_URL}?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=read,activity:read_all&approval_prompt=auto`;
