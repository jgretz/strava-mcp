...
export const getSegment = defineTool({
  ...
  inputSchema: {
    ...
    detail: z
      ...
      .default("summary")
      .describe(
        "summary: one-liner with PR info (default). full: complete segment data as compact JSON.",
      ),
  },
  async handler({ segmentId, detail }) {
    ...
    if (!result.ok) {
      return {
        content: [
          {
            type: "text" as const,
            text: `Failed to get segment: ${result.error}`,
          },
        ],
        isError: true,
      };
    ...
    if (detail === "summary") {
      ...
      if (result.value.athlete_segment_stats) {
        ...
        text += `\nPR: ${formatDuration(stats.pr_elapsed_time ?? 0)} on ${stats.pr_date ?? "unknown"} | ${result.value.effort_count} efforts`;
      }
      return {
    ...
    return {
      content: [
        { type: "text" as const, text: capOutput(compactJson(result.value)) },
      ],
    };
  },
});
