...
export const getActivities = defineTool({
  ...
  inputSchema: {
    ...
    perPage: z.number().optional().describe('Results per page (default 20, max 200)'),
    ...
    detail: z.enum(['basic', 'splits', 'full']).optional().describe('Level of detail: "basic" (default) returns one-liner per activity, "splits" adds splits/laps, "full" returns all fields as JSON'),
  },
  async handler({ page, perPage, before, after, detail }) {
    const limit = perPage ?? 20;
    const result = await listActivities({ page, perPage: limit, before, after });
    if (!result.ok) {
      return {
    ...
    if (level === 'basic') {
      const lines = activities.map((a) => formatActivityLine(a as any));
    ...
    return {
      content: [{ type: 'text' as const, text: capOutput(compactJson(activities)) }],
    };
  },
});
